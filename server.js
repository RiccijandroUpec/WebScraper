...
async function analyzeIntent(userMessage, context) {
  ...
  var systemPrompt = [
    ...
    '',
    'CONTEXTO DE CONVERSACION (si existe):',
    contextStr,
    '',
    'INSTRUCCIONES:',
    '- Analiza el mensaje del usuario TENIENDO EN CUENTA el contexto de la conversacion anterior.',
    '- Si ya tienes informacion del contexto, usala para completar los datos faltantes.',
    '- Responde UNICA Y EXCLUSIVAMENTE con un JSON valido. NINGUN OTRO TEXTO.',
    '',
    'OPERADORAS VALIDAS PARA RECARGAS:',
    '- Claro (Ecuador)',
    '- Movistar (Ecuador)',
    '- CNT (servicio movil)',
    '- Tuenti',
    '- OpenMobile',
    '',
    'SERVICIOS VALIDOS PARA PAGOS:',
    '- CNEL (luz)',
    '- CNT (telefono fijo/internet)',
    '- ETAPA (agua/luz)',
    '- Agua de Quito (EPMAPS)',
    '- Municipio de Guayaquil',
    '- Registro Civil',
    '',
    'FORMATO JSON ESPERADO:',
    '{',
    '  "intent": "topup" | "bill" | "unknown" | "greeting",',
    '  "is_complete": boolean,',
    '  "reply_message": "Mensaje amigable para el usuario",',
    '  "topup_data": {',
    '     "operator": "nombre de la operadora o null",',
    '     "phone": "numero de telefono o null",',
    '     "amount": "monto como string o null"',
    '  },',
    '  "bill_data": {',
    '     "service": "nombre del servicio o null",',
    '     "reference": "numero de referencia o null"',
    '  },',
    '  "missing_fields": ["lista", "de", "campos", "faltantes"]',
    '}',
    '',
    'REGLAS:',
    '1. Si es un saludo (hola, buenos dias, etc) -> intent: "greeting"',
    '2. Para RECARGA necesitas: operadora, celular (10 digitos sin codigo pais) y monto.',
    '3. Para PAGO DE SERVICIO necesitas: nombre del servicio y referencia (cedula o contrato).',
    '4. Si faltan datos, pon "is_complete": false, explica que falta en "reply_message" y lista los campos en "missing_fields".',
    '5. Si completaste con datos del contexto, pon "is_complete": true y confirma en "reply_message".',
    '6. Numeros de telefono: siempre 10 digitos, sin +593, sin espacios.',
    '7. Montos: solo numeros, sin simbolos.',
    '8. IMPORTANTE: Si el usuario dice "quiero recargar" sin dar datos, pidele los 3 datos en la respuesta.'
  ].join('\\n');
